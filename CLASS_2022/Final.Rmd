...
# We wanted to examine the difference between lncRNA and mRNA clustering w/respect to POLII S5/S2

```{r lncRNA promoter clustering}
# Hirano
# Used to build the peak occurrence matrix, w/ init filter for incomplete data
#promoter_peak_occurence <- count_peaks_per_feature(lncrna_mrna_promoters, filtered_consensus_peaks, 
#                                               type = "occurrence")
#stopifnot(all(colnames(promoter_peak_occurence) == lncrna_mrna_promoters$gene_id))
...
For both plots, POLR2A S2 is flanked by ASR2L above and POL2A below. POLR2A S5 is flanked by ZFN652 above and PAF1 below.

Both of the phosphorylated POLR2A genes diverge at the same location between mRNA and lncRNA, suggesting no preference
in binding affinity for either.
